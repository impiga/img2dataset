...
class LoggerProcess(multiprocessing.context.SpawnProcess):
    ...
    def __init__(self, output_folder, enable_wandb, wandb_project, config_parameters, log_interval=5, wandb_name=None):
        super().__init__()
    ...
    def run(self):
        ...
        if self.enable_wandb:
            if self.wandb_name:
                self.current_run = wandb.init(project=self.wandb_project, config=self.config_parameters, anonymous="allow", name=self.wandb_name)
            else:
